...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        # Filling up my dictionary
        for index in dict_shift.keys():
            palabras = 0
            dec_message = ciphertext.apply_shift(index)
            dec_message = dec_message.split()
            for i in dec_message:
                if is_word(wordlist,i):
                    palabras += 1
            dict_shift[index] = palabras
        
        # Extracting highest value
        maxv = 0
        shiftv = 0
        for index in dict_shift.keys():
            if dict_shift[index] > maxv:
                maxv = dict_shift[index]
                shiftv = index
        
        # Final output
        dec_message = ciphertext.apply_shift(shiftv)
        return shiftv, dec_message
...
def decrypt_story():
    ...
    [shift, stringout] = ciphertext.decrypt_message()
    return shift, stringout
